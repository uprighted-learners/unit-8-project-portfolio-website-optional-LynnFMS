import React from 'react'

function Projects() {
  return (
    <div className='Projects'>

<h1>Project Links</h1>
      <ul>
        <li><a href="https://github.com/LynnFMS/Jeopardy.git" target="_blank" rel="noopener noreferrer">Jeopardy</a></li>
        <li><a href="https://github.com/LynnFMS/Guess-the-Number.git" target="_blank" rel="noopener noreferrer">Guess the Number</a></li>
        <li><a href="https://github.com/LynnFMS/Zorkington.git" target="_blank" rel="noopener noreferrer">Zorkington</a></li>
        <li><a href="https://github.com/LynnFMS/Server-Blog.git" target="_blank" rel="noopener noreferrer">Server Blog</a></li>
        <li><a href="https://github.com/LynnFMS/React-Chat" target="_blank" rel="noopener noreferrer">React Chat</a></li>
      </ul>
      

      <h1>Lab Links</h1>
      <ul>
        <li><a href="https://github.com/uprighted-learners/unit-1-project-jeopardy-LynnFMS" target="_blank" rel="noopener noreferrer">Jeopardy</a></li>
        <li><a href="https://github.com/uprighted-learners/unit-2-project-guess-the-number-LynnFMS" target="_blank" rel="noopener noreferrer">Guess the Number</a></li>
        <li><a href="https://github.com/uprighted-learners/unit-3-project-zorkington-LynnFMS" target="_blank" rel="noopener noreferrer">Zorkington</a></li>
        <li><a href="https://github.com/uprighted-learners/unit-5-project-server-blog-LynnFMS" target="_blank" rel="noopener noreferrer">Server Blog</a></li>
        <li><a href="https://github.com/uprighted-learners/unit-6-project-react-chat-RolusC5B" target="_blank" rel="noopener noreferrer">React Chat</a></li>
      </ul>
      </div>

  
  )
}

export default Projects